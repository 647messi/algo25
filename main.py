...
core_stock_id = {0,5,7,8,10,12,15,16,17,18,27,29,30,46,48}
# Signal Count
signal_count = np.zeros(nInst, dtype=int)
last_signal_dir = np.zeros(nInst, dtype=int)
...
def ma_strategy(prcSoFar: np.ndarray, dollar_limit: float = 6000) -> np.ndarray:
    ...
    for i in range(nInst):
        signal = int(np.sign(final_signal[i]))
        last_dir = last_signal_dir[i]
        price = last_prices[i]

        # Reset signal count if direction changes
        if signal != 0 and signal != last_dir and current_position[i] != 0:
            target_position[i] = 0
            signal_count[i] = 0

        # Increse signal count if direction is the same
        if signal != 0:
    ...
    # Stop-loss and take-profit logic with Bollinger Bands
